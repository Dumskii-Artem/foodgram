...
```
Должна быть доступна [Админка]( http://127.0.0.1:8000/admin)
...
Чтобы увидеть работу фронтенда, нужно одновременно запустить фронтенд и бэкенд. Для этого можно испрользовать, например, 2 экземпляра VSCode. 

В одном мы уже запустили сервер backend (если остановили, запустите снова), в другом VSCode откроем папку /frontend/ и выполним
...
Этот вариант на Windows просто так не работает, нужно установить WSL 2 и docker
Для тех, кто учится на этом курсе - не проблема

На Линуксе (на котором я выполнял проект) это работает
